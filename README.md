...
```
## Dataset
The experiments are conducted using the Multimodal University of Bologna (MULB) Biometric Dataset, iris and hand modalities are used.
...
## Train
### Unimodal Iris Siamese Model
```bash
Open and run:
iris-unimodal-code.ipynb
```
### Unimodal Hand (Palm) Siamese Model
```bash
Open and run:
palm-unimodal-code.ipynb
```
### Proposed Multimodal (Hand + Iris) Model with Gated Fusion
```bash
Open and run:
proposed-multimodal-model.ipynb
```
